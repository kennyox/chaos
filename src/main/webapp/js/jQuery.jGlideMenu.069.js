...
jQuery.jGlideMenu = {
	...
	initialize : function(o)
	{
		// Return jQuery
			...
			jQuery(jQuery.jGlideMenu.currentElement)
			//.append('<div class="jGM_header"><a href="#">'+jQuery.jGlideMenu.closeLinkMarkUp+'</a></div>')
	...
	loadTile : function(u,p)
	{
		...
		if(jQuery.jGlideMenu.slideRight == true)
			off = jQuery.jGlideMenu.tileWidth * jQuery.jGlideMenu.tileCount + jQuery.jGlideMenu.tileInset;
		else if(jQuery.jGlideMenu.slideUp == true)
			offUp = jQuery.jGlideMenu.tileHeight * jQuery.jGlideMenu.tileCount + jQuery.jGlideMenu.tileInset;
		else
			off = jQuery.jGlideMenu.tileWidth * jQuery.jGlideMenu.tileCount * -1 + jQuery.jGlideMenu.tileInset;
		...
		else
		{
			...
			p.tile = u;
			var url = jQuery.jGlideMenu.tileSource + u + jQuery.jGlideMenu.URLSuffix;
			jQuery.ajax({
				type: "POST",
				url		: url,
				data		: p,
				...
				success 	: function(xhtml){ 
							...
							var ul = '#jGM_temp_'+jQuery.jGlideMenu.currentElementID+p.tile+' ul#'+u+jQuery.jGlideMenu.type; 
							if(jQuery(ul).size()<1)
                        				{
                                				jQuery.jGlideMenu.errorTrap('AJAX: Invalid Tile Request');
                                				return false;
                        				}
                        				var title = jQuery(ul).attr('title');
                        				var desc  = jQuery(ul).attr('alt');
                        				var links = [];
                        				jQuery(ul+' li').each(function(){
								if(jQuery('a',this).size()>0)
				                                        links[links.length] = [jQuery('a',this).attr('href'),jQuery('a',this).text(),1,jQuery(this).attr('title')];
                                				else
                                        				links[links.length] = [jQuery(this).attr('rel'),jQuery(this).text(),0,jQuery(this).attr('title')];
                        				});
		...
		jQuery('#'+ptr.id+' div.jGM_content .tag').bind('click',function() {
				var target='';
				target = jQuery.jGlideMenu.parseURL(jQuery(this).attr('id'));
		...
		jQuery.jGlideMenu.afterLoadTile(u);
	},
	...
	scrollToTile : function(n,s)
	{
		...
		if(!jQuery.jGlideMenu.slideUp ){
			...
			var b = (jQuery.jGlideMenu.tileHeight * n) - jQuery.jGlideMenu.tileHeight;
			var a = b*-1;
			// Animate
			jQuery.jGlideMenu.animation = true;
			jQuery('div#jGM_wrapper_'+jQuery.jGlideMenu.currentElementID).animate({ 'top' : a }, s, jQuery.jGlideMenu.easeFx, function(){
					// Remove Extra Tiles
					if(n<t)
						jQuery.jGlideMenu.cleanTiles(n);
					if(a != 0) a += 'px';
					jQuery(this).css({ 'top' : a });
	...
	buildTile : function(t,d,l)
	{
		...
		// Header Layout
		if(jQuery.jGlideMenu.showTitle){
		...
		for(var i=0;i<l.length;i++)
		{
			...
			template += '&nbsp;<span class="tag" title="'+l[i][3] +'" id="'+hash+l[i][0]+'"'+type+'>'+l[i][1]+'</span>';
	...
	// after load event
	afterLoadTile : function(tagName)
	{
		if(jQuery.jGlideMenu.afterLoad != null){
			jQuery.jGlideMenu.afterLoad(tagName);
		}
	}
};
